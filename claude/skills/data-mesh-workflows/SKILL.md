...
2. **Add connector to project:** `list_connector_types` → `create_connector` (API key) or `get_connector_oauth_url` (OAuth) → use connector_id in `create_pipeline` or add connection node via `patch_pipeline_graph`.
3. **Apply existing blueprint:** `list_blueprints` (optional) → `apply_blueprint` with project_id, blueprint_type, blueprint_slug.
