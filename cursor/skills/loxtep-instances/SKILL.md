...
**Validation:** `create_instance` rejects the same invalid combinations you would get from the product API (e.g. `shared` + paid `plan_id`, managed without `payment_method_id`, self-hosted without the required Observe / connection details). Use the tool error text and ListTools field descriptions to correct the payload.
